...
class Receiver3(object):
    ...
    def update_newfile_bytearray(self):
        ...
        while True:
            ...
            self.currSeq = int.from_bytes(self.data[:2],'big',signed=False)

            if not self.data:
                break
        
            if (self.currSeq == self.prevSeq):
                self.prevSeq += 1
                ...
                if (self.prevSeq == 0):
                    ...
                    placeholder = self.prevSeq - 1

                ack = bytearray((placeholder).to_bytes(2,'big'))
